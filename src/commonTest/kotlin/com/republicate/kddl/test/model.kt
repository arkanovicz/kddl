...
class BigSerialTest {
    ...
    @Test
    fun testParsesAsPrimitive() {
        val db = parse(CharStreams.fromString(ddl))
        val pk = db.schemas["s"]!!.tables["account"]!!.fields["id"]!!.type
        assertTrue(pk is FieldType.Primitive && pk.name == "bigserial", "expected bigserial primitive, got $pk")
    }

    @Test
    fun testForeignKeyDemotesToBigint() {
        // a bigserial pk (int8) must be referenced by a bigint fk, not an int
        val db = parse(CharStreams.fromString(ddl))
        val fk = db.schemas["s"]!!.tables["entry"]!!.fields["account_id"]!!.type
        assertTrue(fk is FieldType.Primitive && fk.name == "bigint", "expected bigint fk, got $fk")
    }

    @Test
    fun testPostgresqlEmitsBigserial() {
        val sql = com.republicate.kddl.postgresql.PostgreSQLFormatter(false, false, false)
            .format(parse(CharStreams.fromString(ddl)))
        assertTrue(sql.contains("id bigserial"), "expected bigserial column in:\n$sql")
        assertTrue(sql.contains("account_id bigint"), "expected bigint fk column in:\n$sql")
    }

    @Test
    fun testHyperSqlEmitsBigintIdentity() {
        val sql = com.republicate.kddl.hypersql.HyperSQLFormatter(false, false, false)
            .format(parse(CharStreams.fromString(ddl)))
        assertTrue(sql.contains("bigint generated by default as identity"),
